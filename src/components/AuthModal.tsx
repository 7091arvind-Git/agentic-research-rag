...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-3 sm:p-4">
      <div className="relative w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-4 sm:p-7 shadow-2xl border border-slate-200 dark:border-slate-800 dark:bg-slate-900">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="mb-5">
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50/80 px-4 py-2.5 text-xs font-bold uppercase tracking-wider text-indigo-700 hover:bg-indigo-100 transition-colors shadow-xs"
          >
            <Zap className="h-3.5 w-3.5 fill-indigo-600 text-indigo-600" />
            <span>1-Click Test: Sign In as Demo Student</span>
          </button>
          <div className="relative my-4">
            ...
            <div className="relative flex justify-center text-[11px]">
              <span className="bg-white px-2 text-slate-400 font-bold uppercase tracking-wider">or continue with credentials</span>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-300 py-2.5 pl-10 pr-3.5 text-sm text-slate-900 placeholder:text-slate-400 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 font-medium"
                />
              </div>
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-slate-900 py-3 text-xs font-bold uppercase tracking-wider text-white shadow-xs hover:bg-slate-800 disabled:opacity-60 transition-all mt-5"
          >
            {loading ? (
              <>
